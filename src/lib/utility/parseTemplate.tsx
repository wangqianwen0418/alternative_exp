...
/**
 * lib/utility/parseTemplate
 *
 * Renders a human-readable "templated" sentence for a parsed `TInsight`.
 *
 * The returned JSX uses spans with classNames so the UI can highlight parts of
 * the sentence (feature names, relations, constants, etc.).
 */

/**
 * Given a `TInsight`, returns a formatted JSX snippet describing it.
 *
 * If the insight is `undefined` or doesn't match expected categories, a fallback
 * message is returned.
 */
export const GenerateTextTemplates = (insight: TInsight) => {
  // If parsing failed, render an empty string to keep the caller simple.
  if (!insight) return '';
