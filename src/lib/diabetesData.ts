 ...
 * lib/diabetesData
 *
 * Convenience accessors for the precomputed diabetes SHAP dataset.
 *
 * This module:
 * - Loads the bundled `shap_diabetes.json` file.
 * - Extracts per-feature arrays for feature values and SHAP (contribution)
 *   values.
 * - Exposes grouped lists (`diabetesShapValues`, `diabetesFeatureValues`,
 *   `diabetesLabels`) used by chart components.
 * - Provides a simple `variableMapping` used to populate dropdowns and
 *   translate user selections into numeric arrays.
 */
...
export const diabetes_s1 = 'serum cholesterol',
  diabetes_s1_featureIndex =
    shap_diabetes['feature_names'].indexOf(diabetes_s1),
...
export const diabetes_s4 = 'total/HDL cholesterol ratio',
  diabetes_s4_featureIndex =
    shap_diabetes['feature_names'].indexOf(diabetes_s4),
...
/**
 * UI label -> numeric series mapping.
 *
 * Dropdowns/selectors in the UI use the keys of this object; the values are the
 * corresponding arrays in the dataset.
 */
