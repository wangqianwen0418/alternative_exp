...
export default function Scatter(props: ScatterProps) {
    ...
    useEffect(
        ...
        const xAxisGroup = d3.select(`g.scatter#${id}`).append("g")
        .attr("class", 'x-axis')
        .attr("transform", `translate(0,${height})`)

        xAxisGroup.call(d3.axisBottom(xScale))
        xAxisGroup.append("text").attr("text-anchor", "middle")
        ...
        const yAxisGroup = d3.select(`g.scatter#${id}`).append("g")
        .attr("class", 'y-axis')
        ...
        .attr('fill', 'black')
        .text("SHAP Values of BMI");

        }, 
        [xValues, height, width]
    )

    return <g className="scatter" id={id} transform={`translate(${offsets[0]}, ${offsets[1]})`}>
        <rect className="background" width={width} height={height} fill="white" stroke="black"/>
        <g className="points">
            {xValues.map((x, i) => {
                return <circle cx={xScale(x)} cy={yScale(yValues[i])} r={3} stroke='steelblue' fill="none" opacity={0.8} />
            })}
        </g>
